...
void experimento_2(){
    ...
    vector<int> alpha = {0,1,2,3,4};//Trait variables

    // Register of information for bs=bt vs front(s) position(s) graphs
    
    vector<float> mean_front_95(alpha.size(),0); 
    vector<float> sd_front_95(alpha.size(),0); 

    vector<float> mean_front_95CSC(alpha.size(),0); 
    vector<float> sd_front_95CSC(alpha.size(),0); 

    vector<float> mean_front_95TD(alpha.size(),0);
    vector<float> sd_front_95TD(alpha.size(),0); 

    vector<float> mean_front_Naive(alpha.size(),0); 
    vector<float> sd_front_Naive(alpha.size(),0); 

    vector<float> mean_front_NaiveCSC(alpha.size(),0); 
    vector<float> sd_front_NaiveCSC(alpha.size(),0); 

    vector<float> mean_front_NaiveTD(alpha.size(),0);
    vector<float> sd_front_NaiveTD(alpha.size(),0); 

    vector<float> mean_front_zeros(alpha.size(),0); 
    vector<float> sd_front_zeros(alpha.size(),0); 

    vector<float> mean_front_zerosCSC(alpha.size(),0); 
    vector<float> sd_front_zerosCSC(alpha.size(),0); 

    vector<float> mean_front_zerosTD(alpha.size(),0);
    vector<float> sd_front_zerosTD(alpha.size(),0); 

    // Register of information for bs=bt vs front(s) form(s) graphs (dom mode)

    vector<float> mean_dom_mode_95(alpha.size(),0); 
    vector<float> sd_dom_mode_95(alpha.size(),0); 

    vector<float> mean_dom_mode_95CSC(alpha.size(),0); 
    vector<float> sd_dom_mode_95CSC(alpha.size(),0); 

    vector<float> mean_dom_mode_95TD(alpha.size(),0);
    vector<float> sd_dom_mode_95TD(alpha.size(),0); 

    vector<float> mean_dom_mode_zeros(alpha.size(),0); 
    vector<float> sd_dom_mode_zeros(alpha.size(),0); 

    vector<float> mean_dom_mode_zerosCSC(alpha.size(),0); 
    vector<float> sd_dom_mode_zerosCSC(alpha.size(),0); 

    vector<float> mean_dom_mode_zerosTD(alpha.size(),0);
    vector<float> sd_dom_mode_zerosTD(alpha.size(),0); 

    // Register of information for bs=bt vs front(s) form(s) graphs (leght of polygonal)

    vector<float> mean_leght_poly_95(alpha.size(),0); 
    vector<float> sd_leght_poly_95(alpha.size(),0); 

    vector<float> mean_leght_poly_95CSC(alpha.size(),0); 
    vector<float> sd_leght_poly_95CSC(alpha.size(),0); 

    vector<float> mean_leght_poly_95TD(alpha.size(),0);
    vector<float> sd_leght_poly_95TD(alpha.size(),0); 

    vector<float> mean_leght_poly_zeros(alpha.size(),0); 
    vector<float> sd_leght_poly_zeros(alpha.size(),0); 

    vector<float> mean_leght_poly_zerosCSC(alpha.size(),0); 
    vector<float> sd_leght_poly_zerosCSC(alpha.size(),0); 

    vector<float> mean_leght_poly_zerosTD(alpha.size(),0);
    vector<float> sd_leght_poly_zerosTD(alpha.size(),0); 
    ...
    for(int j = 0; j < alpha.size(); j++){
        ...
        for(int k = 0; k < rep; k++){
            LGCA mundo = LGCA(altura, 50, vecbS, vecbS, nu, alpha[j], psym, death);
            mundo.putstrip();
            mundo.walk(Time);
            mundo.draw_line();
        ...
        // Register of information for alpha vs front(s) position(s) graphs
    ...
    // Register of information for alpha vs front(s) form(s) graphs (dom mode)
    ...
    // Register of information for alpha vs front(s) form(s) graphs (leght of polygonal)
    ...
    ofstream data_info (path+"sim002/sim002.py", ios_base::app);
    ...
    data_info << mean_front_95[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_95[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_95CSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_95CSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_95TD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_95TD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_Naive[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_Naive[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_NaiveCSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_NaiveCSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_NaiveTD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_NaiveTD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
//*********************************************************
    ...
    data_info << mean_front_zeros[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_zeros[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_zerosCSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_zerosCSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_front_zerosTD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_front_zerosTD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
//*********************************************************
    ...
    data_info << mean_dom_mode_95[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_dom_mode_95[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_dom_mode_95CSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_dom_mode_95CSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_dom_mode_95TD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_dom_mode_95TD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_dom_mode_zeros[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_dom_mode_zeros[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_dom_mode_zerosCSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_dom_mode_zerosCSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_dom_mode_zerosTD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_dom_mode_zerosTD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
//*********************************************************
    ...
    data_info << mean_leght_poly_95[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_leght_poly_95[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_leght_poly_95CSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_leght_poly_95CSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_leght_poly_95TD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_leght_poly_95TD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_leght_poly_zeros[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_leght_poly_zeros[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_leght_poly_zerosCSC[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_leght_poly_zerosCSC[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    data_info << mean_leght_poly_zerosTD[alpha.size()-1] << " ]" <<endl;
    ...
    data_info << sd_leght_poly_zerosTD[alpha.size()-1] << " ]" <<endl;
    //-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
